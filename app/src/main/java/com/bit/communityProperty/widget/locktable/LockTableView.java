...
public class LockTableView {
     ...
     * 表格数据，每一行为一条数据，从表头计算
     */
    private ArrayList<ArrayList<WorkTime>> mTableDatas = new ArrayList<ArrayList<WorkTime>>();
    /**
     * 表格视图
     */
     ...
     * 表格第一行数据,不包括第一个元素
     */
    private ArrayList<WorkTime> mTableFristData = new ArrayList<>();
    /**
     * 表格第一列数据，不包括第一个元素
     */
    private ArrayList<WorkTime> mTableColumnDatas = new ArrayList<>();
    /**
     * 表格左上角数据
     */
     ...
     * 表格每一行数据，不包括第一行和第一列
     */
    private ArrayList<ArrayList<WorkTime>> mTableRowDatas = new ArrayList<ArrayList<WorkTime>>();
    /**
     * 记录每列最大宽度
     */
     ...
     */
    public LockTableView(Context mContext, ViewGroup mContentView, ArrayList<ArrayList<WorkTime>> mTableDatas) {
    ...
    private void initData() {
        ...
        if (mTableDatas != null && mTableDatas.size() > 0) {
            ...
            for (int i = 0; i < mTableDatas.size(); i++) {
                ...
                ArrayList<WorkTime> rowDatas = mTableDatas.get(i);
                for (int j = 0; j < rowDatas.size(); j++) {
                    if (rowDatas.get(j) == null || rowDatas.get(j).getWorkContent()==null || rowDatas.get(j).getWorkContent().equals("")) {
                        rowDatas.set(j, workTime);
                    }
                }
...
//            }
            ...
            for (int i = 0; i < mTableDatas.size(); i++) {
                ...
                for (int j = 0; j < rowDatas.size(); j++) {
                    ...
                    textView.setText(rowDatas.get(j).getWorkContent());
                    textView.setGravity(Gravity.CENTER);
                    //设置布局
                    LinearLayout.LayoutParams textViewParams = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,
                            LinearLayout.LayoutParams.WRAP_CONTENT);
                    textViewParams.setMargins(35, 35, 35, 35);//android:layout_margin="15dp"
                    textView.setLayoutParams(textViewParams);
                    if (i == 0) {
                        mColumnMaxWidths.add(measureTextWidth(textView, rowDatas.get(j).getWorkContent()));
                        buffer.append("[" + measureTextWidth(textView, rowDatas.get(j).getWorkContent()) + "]");
                    } else {
                        int length = mColumnMaxWidths.get(j);
                        int current = measureTextWidth(textView, rowDatas.get(j).getWorkContent());
                        if (current > length) {
...
//            Log.e("每列最大宽度dp:",mColumnMaxWidths.toString());
            ...
            for (int i = 0; i < mTableDatas.size(); i++) {
                ...
                textViewParams.setMargins(35, 35, 35, 35);//android:layout_margin="15dp"
                textView.setLayoutParams(textViewParams);
                int maxHeight = measureTextHeight(textView, rowDatas.get(0).getWorkContent());
                mRowMaxHeights.add(maxHeight);
                for (int j = 0; j < rowDatas.size(); j++) {
                    int currentHeight = measureTextHeight(textView, rowDatas.get(j).getWorkContent());
...
//            Log.e("每行最大高度dp:",mRowMaxHeights.toString());
            ...
            if (isLockFristRow) {
                ArrayList<WorkTime> fristRowDatas = (ArrayList<WorkTime>) mTableDatas.get(0).clone();
                if (isLockFristColumn) {
                    //锁定第一列
                    mColumnTitle = fristRowDatas.get(0).getWorkContent();
                    fristRowDatas.remove(0);
                    mTableFristData.addAll(fristRowDatas);
                    //构造第一列数据,并且构造表格每行数据
                    for (int i = 1; i < mTableDatas.size(); i++) {
                        ArrayList<WorkTime> rowDatas = (ArrayList<WorkTime>) mTableDatas.get(i).clone();
...
//                Log.e("横向滚动监听","["+x+"] "+"["+y+"]");
     ...
     */
    private void createScollview(HorizontalScrollView scrollView, List<WorkTime> datas, boolean isFristRow) {
        //设置LinearLayout
...
//                textView.setTextColor(ContextCompat.getColor(mContext, textColor[datas.get(i).getType()]));
     ...
     * @param mTableDatas
     */
    public void setTableDatas(ArrayList<ArrayList<WorkTime>> mTableDatas) {
        this.mTableDatas = mTableDatas;
    ...
    public interface OnLoadingListener {
         ...
         * @param mTableDatas
         */
        void onRefresh(XRecyclerView mXRecyclerView, ArrayList<ArrayList<WorkTime>> mTableDatas);
         ...
         * @param mTableDatas
         */
        void onLoadMore(XRecyclerView mXRecyclerView, ArrayList<ArrayList<WorkTime>> mTableDatas);
    }
}
